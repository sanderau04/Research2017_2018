%% correlation_externalTmerged.m performs initial spearman correlations with 
% PDx externalTmerged and sumamry features ONLY. 
...
for i = 1:length(matData)
    externalTmerged(i) = matData(i).patientDx(70);
...
%% Run spearman correlations on features and externalTmerged

[rho(1), pval(1)] = corr(avgSPDomFreq',externalTmerged', 'type', 'spearman');
[rho(2), pval(2)] = corr(avgSPMeanFreq', externalTmerged', 'type', 'spearman');

x = 3;
for w=1:14
   [rho(x), pval(x)] = corr(summaryFeatures(:,w),externalTmerged','type','spearman');
   x = x + 1;
end
...
extTmergedCorrTable = table(rho',pval','VariableNames', Variables, 'RowNames', RowNames);
