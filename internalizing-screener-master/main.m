...
%% Load data
clear all, clc
dt = readtable('FeaturesP2_8_10.xlsx');
%rankedSigCorrT = readtable('Features_8_6/RankedSignificance_FromCorr_Part1.xlsx');
%rankedSigCorr = table2array(rankedSigCorrT);
%{
TF = ismissing(dt_raw);
dt = dt_raw(~any(TF,2),:);
%}
...
%{
for w=1:N
    TopNFeatures(w) = string(dt.Properties.VariableNames{selectedFeats(w)});
end
TopNFeatures = TopNFeatures';


for w=1:length(rankedSigCorr)
    if rankedSigCorr(w) == TopNFeatures(w)
        indexMatch(w,1) = w;
        indexMatch(w,2) = w;
    else
        for u=1:N
            if rankedSigCorr(w) == TopNFeatures(u)
                indexMatch(w,1) = w;
                indexMatch(w,2) = u;
            end
        end
...
selectedFeaturesTable.Properties.VariableNames = {'Selected_Features_N5', 'Feature_Count_40_Iterations'};
