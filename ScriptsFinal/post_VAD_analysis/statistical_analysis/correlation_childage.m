%% correlation_childage.m performs initial spearman correlations with 
% PDx childage and sumamry features ONLY.
...
for i = 1:length(matData)
    childage(i) = matData(i).patientDx(29);
...
%% Run spearman correlations on features and childage



[rho(1), pval(1)] = corr(avgSPDomFreq',childage', 'type', 'spearman');
[rho(2), pval(2)] = corr(avgSPMeanFreq', childage', 'type', 'spearman');

x = 3;

for w=1:14
   [rho(x), pval(x)] = corr(summaryFeatures(:,w),childage','type','spearman');
   x = x + 1;
end
...
ageCorrTable = table(rho',pval','VariableNames', Variables, 'RowNames', RowNames);
