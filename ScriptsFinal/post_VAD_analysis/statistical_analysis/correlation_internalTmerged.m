%% correlation_internalTmerged.m performs initial spearman correlations 
% with  PDx interalTmerged and sumamry features ONLY. 
...
for i = 1:length(matData)
    internalTmerged(i) = matData(i).patientDx(46);
...
%internalTmerged = rmmissing(internalTmerged); %debug for missing
%internalTmerged values


%% Run spearman correlations on features and internalTmerged
[rho(1), pval(1)] = corr(avgSPDomFreq',internalTmerged', 'type', 'spearman');
[rho(2), pval(2)] = corr(avgSPMeanFreq', internalTmerged', 'type', 'spearman');

x = 3;
for w=1:14
   [rho(x), pval(x)] = corr(summaryFeatures(:,w),internalTmerged','type','spearman');
   x = x + 1;
end
...
intTmergedCorrTable = table(rho',pval','VariableNames', Variables, 'RowNames', RowNames);
